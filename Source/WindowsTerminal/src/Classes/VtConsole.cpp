...
PCWSTR GetCmdLine()
{
    // return L"D:\\Applications\\Scoop\\shims\\nvim.exe";
    return L"C:\\Windows\\System32\\conhost.exe";
}
...
void VtConsole::_createPseudoConsole(const std::wstring& command)
{
    ...
    if (realCommand == L"")
    {
        // realCommand = L"C:\\Windows\\System32\\conhost.exe";
        realCommand = L"C:\\Windows\\System32\\conhost.exe --headless";
        // realCommand = L"D:\\Applications\\Scoop\\shims\\nvim.exe";
...
void VtConsole::_createConptyViaCommandline(const std::wstring& command)
{
    ...
    // cmdline = L"D:\\Applications\\Scoop\\shims\\nvim.exe";
...
std::string ReadPipe(void* ReadPipe)
{
    std::string Output;

    // Note: String becomes corrupted when more than one byte per character and all bytes are not available
    DWORD BytesAvailable = 0;
    if (::PeekNamedPipe(ReadPipe, NULL, 0, NULL, &BytesAvailable, NULL) && (BytesAvailable > 0))
    {
        std::vector<char> Buffer(BytesAvailable + 1);
        DWORD BytesRead = 0;
        if (::ReadFile(ReadPipe, Buffer.data(), BytesAvailable, &BytesRead, NULL))
        {
            if (BytesRead > 0)
            {
                Buffer[BytesRead] = '\0';
