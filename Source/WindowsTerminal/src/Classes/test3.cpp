...
////////////////////////////////////////////////////////////////////////////////
///
FString ReadPipe( void* ReadPipe )
{
    FString Output;
...
void newConsole()
{
    auto con = new VtConsole(ReadCallback, true , true , { 50 , 50 });
    con->spawn();
    consoles.push_back(con);
}
...
FString CreateChildProcessAndGetOutput(const FString& ExecutablePath, const FString& Parameters)
{

    
    newConsole();
    getConsole()->activate();
    auto ss2 = ReadPipe(getConsole()->outPipe());
    std::cout<< TCHAR_TO_UTF8(*ss2)<< std::endl;
    // CreateIOThreads();
    Sleep(10.f);
    // std::string i("nvim\r");
    std::string i("echo Hello, World!\r");
    getConsole()->WriteInput(i);

    // WriteToPipe(hChildStd_IN_Wr, "echo Hello, World!\n");

    // FWindowsPlatformProcess::ReadPipe()
    auto ss = ReadPipe(getConsole()->outPipe());
    std::cout<< TCHAR_TO_UTF8(*ss)<< std::endl;
    ...
    //     bool fSuccess = false;
    //
    //     fSuccess = !!ReadFile(getConsole()->outPipe(), buffer, ARRAYSIZE(buffer), &dwRead, nullptr);
    //     if (!fSuccess)
    //     {
    //         HRESULT hr = GetLastError();
    //         // exit(hr);
